...
class NewSchemaReader extends SchemaReader {
  ...
  private void loadSources() throws Exception {
    ...
    try (Statement st = conn.createStatement();
         ResultSet rs = st.executeQuery(
             "SELECT id, name, abbreviated_name, authors_and_editors, organisation, version, release_date FROM source_database")) {
      while (rs.next()) {
        addSourceCitation("d" + rs.getInt("id"), rs.getString("name"), rs.getString("abbreviated_name"),

...
**Output:** `NameUsage` (accepted + synonyms), `VernacularName`, `Distribution`, `Reference`, and a `metadata.yaml` whose `source:` registry lists every contributing Global Species Database (GSD). Each NameUsage carries a `sourceID` pointing to its GSD. Distribution `establishmentMeans` keeps the raw source label (native, alien, domesticated, …) for ChecklistBank to normalise on import.

**Metadata** is per-year: `alias` = `COL{yy}`, `title` = "Catalogue of Life Checklist", `version` = "Annual Checklist {year}", `url` = `https://www.catalogueoflife.org/annual-checklist/{year}/`, and creator = organisation *Species 2000 & ITIS Catalogue of Life* (Reading, GB through 2013; Leiden, NL from 2014) plus the per-year editor list with ORCIDs where known. Editor lists come from the archived "How to cite this work" pages kept under `src/main/resources/colac/citations/` (2005 and 2010–2019; 2006–2009 list Bisby & Roskov, who led throughout). `issn` is `1473-009X` for 2005–2015, `2405-884X` for 2016/2017/2019, and `2405-917X` for 2018.

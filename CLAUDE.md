...
**Metadata** (shared template `resources/colac/metadata.yaml`, filled by `Generator.addMetadata()`): `alias = COL{2-digit year}` (e.g. COL05), `title = Catalogue of Life Checklist`, `version = Annual Checklist {year}`, `issued` = latest GSD release date, `url = https://www.catalogueoflife.org/annual-checklist/{year}/`. The single `creator` is the organisation *Species 2000 & ITIS Catalogue of Life* with city/country **Reading, GB** (≤2013) or **Leiden, NL** (≥2014) — Species 2000 moved to Leiden in 2013, first Leiden release was 2014 — followed by the per-year editor list. Editors are hardcoded from the published Annual Checklist citations (years 2011–2019 only; 2005–2010 list only the organisation) and parsed into given/family by `ColacMappings.parseEditors` (handles particles like "Nieukerken E. van" → family "van Nieukerken", and multi-word surnames like "De Wever"). `issn` is set for 2016–2019 (2405-884X, except 2018 = 2405-917X, the DVD ISSN).

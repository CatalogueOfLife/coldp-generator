...
**Metadata**: each year has an explicit, hand-editable file `resources/colac/metadata/<year>.yaml` (2000, 2002–2019). These were compiled from two editor's spreadsheets in `resources/colac/citations/` by `scripts/gen_colac_metadata.py`: `ACs_metadata_summary_v1.xlsx` (per-year title/description/scope/editor list/etc.) and `ACs_editors00-19_v1.xlsx` (per-year, per-editor full ColDP **Agent** records — its columns map 1:1 to the Agent type); re-run it only if a spreadsheet or the author map changes, otherwise edit the YAML directly. `Generator.addMetadata()` now only fills `{issued}` (latest GSD / CD-ROM release date) and appends the dynamic `{sources}` GSD registry; everything else (title, description, alias, version, scopes, confidence, completeness, ISSN, keyword, url, contact, publisher, editor, conversion, license) is explicit per file. Per-year facts from the spreadsheet: full per-year **title** and **description**; **publisher** organisation *Species 2000* (uniform across all years 2000 & 2002–2019 — the historical "& ITIS Catalogue of Life" suffix was dropped from the org name per the 2026 editor review, surviving only in each year's **title**) with city/country **Los Baños, PH** (2000–2004), **Reading, GB** (2005–2013), **Leiden, NL** (2014–2019); the annual editors go in ColDP **`editor`** (not `creator`) with initials expanded to full given names. **Editor ORCID + affiliation** come from `ACs_editors00-19_v1.xlsx` via `load_editor_details()`, merged by `(year, family)` — it supplies full per-year `organisation/department/city/state/country` (ISO-coded; source typos fixed: Reasearch→Research, Liebniz→Leibniz, Los-Baños→Los Baños) for every editor in **2000 & 2002–2005** (e.g. Froese ICLARM/Makati 2000 → Institute of Marine Research/Kiel 2002–03 → Leibniz Institute/Kiel 2004), plus ORCIDs for Froese `0000-0001-9745-636X`, Wilson `0000-0001-7419-8222`, Soulier-Perkins `0000-0002-9537-8537`. Its **2006–2019 sheets are empty**, so those years fall back to the `AUTHORS` ORCID map + the script's `affiliation()` (the three editor-confirmed (2026) affiliations: Kimani → Univ. of Reading, GB; Hernandez → VLIZ, Ostend, BE; Paglinawan → WorldFish Center, Los Baños, PH for 2007–2010, then FishBase Information and Research Group (FIN), Los Baños, PH for 2011–2014); **ISSN** `1473-009X` (≤2014) / `2405-884X` (≥2015); **temporalScope** "Extant taxa" (≤2015) / "Extant & extinct taxa" (≥2016); `taxonomicScope: Biota`, `geographicScope: Global`, `confidence: 4`. Added to every year: `keyword: [COL, Catalogue of Life, species list, community]`, `url: http://www.catalogueoflife.org/annual-checklist/<year>/`, `contact:` COL Secretariat (Amsterdam, NL, support@catalogueoflife.org, from dataset/3), and `conversion:` (github url + a short MySQL-export description, noting the MS Access → MySQL step for 2000–2004). The rendered files parse via `ColdpMetadataParser` (the CLB importer's parser).

...
**ID scheme:** `t<id>` accepted taxa (from the `taxa` tree), `a<id>` accepted names missing from the tree (parentless, from `scientific_names`), `s<id>` synonyms (and bare names), `r<id>` references, `d<id>` source databases.
